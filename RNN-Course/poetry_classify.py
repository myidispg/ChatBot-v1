...
class SimpleRNN:
    ...
    def fit(self, X, Y, learning_rate=10e-1, mu=0.99, reg=1.0, activation=T.tanh, epochs=500, show_fig=False):
        ...
        thX, thY, py_x, prediction = self.set_params(Wx, Wh, bh, h0, Wo, bo, activation)
        ...
        for i in range(epochs):
            ...
            print(f'i: {i}; cost: {cost}; correct_rate: {float(n_correct)/N}; validation_correct_rate: {float(n_correct_valid)/N}')
            costs.append(cost)
    ...
    def set_params(self, Wx, Wh, bh, h0, Wo, bo, activation):
        self.f = activation
